...
void Graph::rat_dfs(int index, vector<bool> &visit) {
	...
	for (int i=0; i<2; i++) {
		...
		if (nd.at[EARLY][type] == UNDEFINED_AT[EARLY] || nd.rat[EARLY][type] == UNDEFINED_RAT[EARLY]) nd.slack[EARLY][type] = UNDEFINED_SLACK[EARLY];
		else nd.slack[EARLY][type] = nd.at[EARLY][type] - nd.rat[EARLY][type];
		if (nd.at[LATE][type] == UNDEFINED_AT[LATE] || nd.rat[LATE][type] == UNDEFINED_RAT[LATE]) nd.slack[LATE][type] = UNDEFINED_SLACK[LATE];
		else nd.slack[LATE][type] = nd.rat[LATE][type] - nd.at[LATE][type];
	}
}
